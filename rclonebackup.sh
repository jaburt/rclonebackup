...
exclude_list="/mnt/tank/scripts/rclone_excludes.txt"
...
backupDestination="/mnt/tank/scripts/rclone_logs"
...
# Location of the downloaded "sendemail.py"
...
run_invalid()
{
# Temporary file for the email body, email subject and finished date/time.
invalidbody="/tmp/rcloneinvalidbody.tmp"
invalidsubject="rclone - ERROR: ${1}"
finished=$(date "+ and finished at: %Y-%m-%d %I:%M:%S")

# Define the email text and send it.
	{
	printf "%s%n" "<pre style=\"font-size:14px\">"
	printf "%s\n"
...
run_rclone()
{
successbody="/tmp/rcloneemailbody.tmp"
